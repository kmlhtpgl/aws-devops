...
We can edit files with `-i` option. This time we change all `linux` world with `ubuntu`.

```bash
sed -i 's/linux/ubuntu/gi' sed.txt
```

## Part 2 - awk command

- Awk is a text pattern scanning and processing language, created by Aho, Weinberger & Kernighan (hence the name is AWK). It searches one or more files to see if they contain lines that matches with the specified patterns and then performs the associated actions. 
...
- We can change delimiter by using –F option. Let's separate the fields by `:`.
